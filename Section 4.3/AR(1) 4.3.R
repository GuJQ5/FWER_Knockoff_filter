...
M.sequence = c(1,21,41,61)
for(i in 1:4){
  
  M.19<-18 + M.sequence[i]
...
Power.matrix = matrix(0,4,2000)
FWER.matrix = matrix(0,4,2000)
for (amplitude in amp.seq){
  ...
  for(i in 1:length(M.sequence)){
    ...
    write.csv(FWER_M[[i]],paste0('/oak/stanford/groups/zihuai/yuxinrui/zihuai/simulation_biometrics_summary/Summary_Feb_Power/simulation_M/FWER_ar_4nv_',i,'_rho_',rho,'.csv'),row.names=F,col.names=T,quote=F,sep='\t')
    write.csv(Power_M[[i]],paste0('/oak/stanford/groups/zihuai/yuxinrui/zihuai/simulation_biometrics_summary/Summary_Feb_Power/simulation_M/Power_ar_4nv_',i,'_rho_',rho,'.csv'),row.names=F,col.names=T,quote=F,sep='\t')
...
write.csv(FWER,paste0('/oak/stanford/groups/zihuai/yuxinrui/zihuai/simulation_biometrics_summary/Summary_Feb_Power/simulation_M/FWER_ar_4nv_',p,'_rho_',rho,'.csv'),row.names=F,col.names=T,quote=F,sep='\t')


write.csv(Power,paste0('/oak/stanford/groups/zihuai/yuxinrui/zihuai/simulation_biometrics_summary/Summary_Feb_Power/simulation_M/Power_ar_4nv_',p,'_rho_',rho,'.csv'),row.names=F,col.names=T,quote=F,sep='\t')
